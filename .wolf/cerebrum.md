...
- **Group cache must be pre-populated each reconcile**: `full_reconcile()` calls `api.get_monitors()` at start and builds `group_cache = {m["name"]: m["id"] for m in all_monitors if type == GROUP}`. This warm cache is passed to all `ensure_group()` calls. Without pre-population, groups created in prior reconcile cycles are not found and get re-created (bug-006 — a cross-cycle version of bug-005).
...
- **[2026-04-07]** Do not initialise `group_cache = {}` empty at reconcile start — pre-populate it from `api.get_monitors()` scanning for GROUP-type monitors, otherwise groups are re-created on every reconcile cycle (bug-006).
